...
export function AuthField({ label, icon, password = false, type, ...inputProps }: AuthFieldProps) {
  ...
  return (
    <div className="grid gap-1.5">
      <label className="text-[13px] font-semibold text-[#52617d]" htmlFor={inputProps.id}>
        {label}
      </label>
      <div className="flex min-h-11 items-center gap-2.5 rounded-xl border border-[#e3e8f2] bg-[#fafbfe] px-3.5 transition focus-within:border-[#3f6ee8] focus-within:bg-white focus-within:ring-4 focus-within:ring-[#3f6ee8]/12">
        {icon}
        <input
          className="min-w-0 w-full border-0 bg-transparent text-sm text-[#273452] outline-none placeholder:text-[#a9b2c3]"
          type={password ? (visible ? 'text' : 'password') : type}
          {...inputProps}
        />
        {password && (
          <button
            className="grid cursor-pointer place-items-center border-0 bg-transparent p-0 text-[#8490a9] hover:text-[#3f6ee8]"
            type="button"
...
export function UserIcon() {
  return (
    <svg className="size-[19px] shrink-0 text-[#97a3bc]" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.8">
